...
本思路由:原心·执念 提供，且成功率很高

理论上步骤全做好可适用于全部的GSI。且在执行此部分操作前，你可能需要先安装Magisk和MT管理器，并确保设备获取ROOT权限。

1. 去此链接下载所需要的资源包https://share.feijipan.com/s/FPQ28TS7，下载好后在小爱老师里使用 MT管理器 解压在你能找到的目录
2. 使用 MT 管理器 打开你解压好的文件目录。先将 Google Play 商店 安装包重命名为"Phonesky.apk"(注意大小写)，将其复制(不要剪切)到/data/adb目录下。
3. 打开 microG应用 文件夹，先安装 microG Services Framework Proxy 和 microG Installer Revived overlay (不分先后顺序)，然后在安装 microG 服务_0.3.13 这个应用(注意版本号)，安装好后打开登录你的 Google 账号，登录成功后再安装 microG 服务_0.3.4 ，安装成功后再进行打开看看你登录的 Google 账号还在不在，如果不在，请重新开始第二步。
4. 如果你的 Google 账号存在，再返回MT管理器打开 Google Play 商店 安装包进行安装，当然你安装成功后是打不开的，所以接着下一步。
5. 打开Magisk，点击模块进行安装，定位到你解压的文件夹，找到模块，会发现有两个压缩包，一个是中文版(就是对安装流程全都汉化)，另一个是原版，建议大家安装中文版，可以方便排查错误。如果前四个步骤没做错的话，模块会自动安装完成并要求你进行重启。
6. 重启完成后打开 Google Play 商店 。就可以正常使用了(有时可能第一次会提示更新错误或者失败，跳过或稍后检查就行)

接下来回答部分疑问:
1.问:为什么安装两次版本不同的microG服务？
  答:该教程安装的模块仅支持0.3.4版本的microG，，且0.3.4版本的microG版本登录 Google 账号 很大概率会失败，所以只能使用0.3.13这样的高版本的microG进行登录，0.3.4版本进行 Play服务 使用
